...
import { Banner, Button, Badge, Alert } from "@/components/ui";
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button variant="danger" onClick={revokeAll}>Revoke All Other Sessions</Button>
      </div>
                  ...
                  <div key={s.id} onClick={() => setSelected(p => p?.id === s.id ? null : s)}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {s.current && <Badge variant="success" size="xs">Current</Badge>}
                      </div>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      {!s.current && (
                        <Button variant="danger" size="xs" onClick={e => { e.stopPropagation(); revoke(s.id); }}>
                          Revoke
                        </Button>
                      )}
                    </div>
        ...
        {selected && (
          <div className="w-72 shrink-0 rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="p-4 flex flex-col gap-3">
              ...
              {!selected.current && (
                <Button variant="danger" fullWidth className="mt-2" onClick={() => revoke(selected.id)}>
                  Revoke This Session
                </Button>
              )}
            </div>
      ...
      <Alert variant="info">
        <span className="font-semibold">Security Tip</span> — If you see an unfamiliar session, revoke it immediately and change your password. Consider enabling MFA if not already active.
      </Alert>
    </div>
  );
}
