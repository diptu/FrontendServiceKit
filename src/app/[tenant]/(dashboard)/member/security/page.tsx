...
import {
  ...
  MonitorSmartphone, ShieldAlert, ShieldCheck, X,
} from "lucide-react";
import { MemberCard, MemberStatCard, ActivityItem } from "@/components/member/ui";

const TABS = ["Overview", "MFA Setup", "Login History"] as const;
      ...
      <div className="flex border-b border-slate-200">
        {TABS.map((t) => (
          <button key={t} type="button" onClick={() => setTab(t)}
            className={`px-5 py-2.5 text-sm font-medium transition-colors border-b-2 -mb-px ${tab === t ? "border-indigo-600 text-indigo-600" : "border-transparent text-slate-500 hover:text-slate-700"}`}>
            {t}
          </button>
        ))}
      </div>
      ...
      {tab === "Overview" && (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            <div className="flex flex-col gap-3">
              {methods.map((m, i) => (
                <div key={m.id} className={`flex items-center justify-between gap-3 ${i < methods.length - 1 ? "pb-3 border-b border-slate-100" : ""}`}>
                  ...
                  <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${m.enrolled ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>
          ...
          <MemberCard title="Password & Account">
            ...
            <div className="mt-4">
              <button type="button" onClick={() => setShowPwFields((p) => !p)}
              ...
              {showPwFields && (
                <div className="mt-3 flex flex-col gap-2">
                  ...
                  <button type="button" className="mt-1 rounded-lg bg-indigo-600 py-2 text-xs font-semibold text-white hover:bg-indigo-700">
                    Update Password
                  </button>
                </div>
              )}
