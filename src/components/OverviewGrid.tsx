...
const HEALTH_STYLES: Record<ApiHealthStatus, HealthStyle> = {
  operational: { dot: "bg-green-500",  text: "text-green-700",  label: "Operational" },
  degraded:    { dot: "bg-orange-500", text: "text-orange-700", label: "Degraded"     },
  outage:      { dot: "bg-red-500",    text: "text-red-700",    label: "Outage"       },
};

const DEFAULT_METRICS: OverviewMetric[] = [
  { id: "active-connections", label: "Total Active Connections",  value: "4,218",  icon: Activity,   service: "Session Gateway",        health: "operational" },
  { id: "mfa-pending",        label: "Pending MFA Enrollments",  value: "37",     icon: ShieldAlert, service: "MFA Enrollment Service",  health: "degraded"    },
  { id: "issued-tokens",      label: "Issued Access Tokens",     value: "12,904", icon: KeyRound,   service: "Token Issuance Service",  health: "operational" },
  { id: "avg-risk-score",     label: "Average Session Risk Score", value: "0.18", icon: Gauge,      service: "Risk Scoring Engine",     health: "operational" },
];
          ...
          <StaggerItem key={metric.id}>
            <div className="flex h-full flex-col justify-between rounded-xl border border-gray-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md">
              ...
              <div className="mt-4">
                ...
                <p className="mt-1 text-3xl font-semibold tracking-tight text-slate-800">{metric.value}</p>
              </div>
              ...
              <div className="mt-4 flex items-center gap-2 border-t border-gray-100 pt-3">
                ...
                <span className={`ml-auto text-xs font-semibold ${healthStyle.text}`}>{healthStyle.label}</span>
              </div>
            </div>
          </StaggerItem>
        );
