...
export function DropdownMenu({
  ...
  return (
    <div ref={ref} className={`relative inline-block ${className}`}>
      ...
      >
        {items.map((item, i) => {
          if (item.separator) return <div key={`sep-${i}`} className="my-1 border-t border-slate-100" />;
...
export function KebabMenu({ items, onSelect, align = "right", className = "" }: KebabMenuProps) {
  return (
    <DropdownMenu
      trigger={
        <button type="button" className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition-colors" aria-label="More actions">
